...
def epipolarMatchGUI(I1, I2, F):
    ...
    while True:
        ...
        # save the F and pts1, pts2
        i += 1
        num_pts = 8  # the number of point-pairs we want to draw
        pts1 = np.append(pts1, np.array([xc, yc]).reshape((1, 2)), axis=0)
        pts2 = np.append(pts2, np.array([x2, y2]).reshape((1, 2)), axis=0)
        if i >= num_pts:
            plt.savefig('../data/f1')
            break
